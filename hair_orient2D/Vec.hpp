...
inline long double my_cbrt(long double x)
{
	using namespace std; // For pow
...
inline long double my_round(long double x)
{
...
# undef trunc
#endif
inline float my_trunc(float x)
{
	return (x < 0.0f) ? float(int(x)) : float(int(x));
}
inline double my_trunc(double x)
{
	return (x < 0.0f) ? double(int(x)) : double(int(x));
}
inline long double my_trunc(long double x)
{
