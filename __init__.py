...
class MyPanel(Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.is_link_head:
            box.prop_search(context.scene, "head_object", bpy.data, "objects", text="")
...
class ConvertToHairCurveOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.is_link_head:
            head = bpy.context.scene.head_object
...
def register():
    ...
    bpy.types.Scene.head_object = bpy.props.PointerProperty(type=bpy.types.Object)
...
def unregister():
    ...
    del bpy.types.Scene.head_object
